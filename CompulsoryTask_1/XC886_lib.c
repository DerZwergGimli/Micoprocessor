// Einheitliche Bibliotheks Funktionen f�r den XC886
// Enth�lt Bibliotheksfunktionen f�r:
// LCD-Display - Analog-Digital Wandler - RS232 - I2C Bus

// ------------------------------------------------------------------------------------------------------------------
// Bibliotheks Funktionen f�r den ADC des XC866:
// adcinit() muss einmal aufgerufen werden
// mit wert = ADCIN (kanal)  kann ein Analogwert in 10 Bit
// Aufl�sung eingelesen werden . Kanal = 4 .. 7 m�glich !!
...
unsigned int adc_in(unsigned char kanal)
{	// Nur Kanal 4.. 7 ist m�glich !

	ADC_PAGE = 0x06;
	ADC_CRPR1= 1 << kanal ;	// ANx gew�hlt
	while(ADC_Busy());
	ADC_PAGE = 0x02;
...
// SDA: P3.0, SCL: P3.1
//************************************************************************
#define WARTE 10		// Schleife f�r XC886 von 5 auf 10 erh�ht(ca. 7us)
...
unsigned char byte_in(void)
{
	...
	{
	  SCL = 1;	  				// Daten m�ssen stabil sein
	  SCL = 1;
	  SCL = 1;
      for (k=0;k<WARTE;k++);	// kurz warten
...
bit byte_out(unsigned char wert)
{
    ...
    for (j=0;j<8;j++) 			// 8 Bit
	{
	  if (wert > 127)  SDA = 1;	// MSB pr�fen > 127 dann Bit gesetzt
	  else SDA = 0;
	  wert = wert << 1;
	  SCL = 1;					// SCL=1, dann Daten g�ltig
	  for (k=0;k<WARTE;k++);	// kurz warten
	  SCL = 0;					// Daten d�rfen wechseln
	  for (k=0;k<WARTE;k++);	// kurz warten
	}
...
//************************************************************************
// SDA muss zuerst auf 0 gehen, fr�hestens 4us sp�ter dann SCL
//************************************************************************
bit start_i2c(void)
{
	bit errorbit;
...
void repstart_i2c(void)
{
	P3_DIR |= 0x03;				// SCL und SDA Ausg�nge
	SDA = 1;
	SDA = 1;
	SDA = 1;
	SCL = 1;
...
//**************************************************************************
// Bibliotheksfunktionen f�r den I2C Bus
// *************************************************************************
void rs232_init(void)
{
	PORT_PAGE = 0x02; // Port Page 2  f�r ALT Select
   	P1_ALTSEL1 = 0x02;   //Alternative Portbelegung
   	P1_ALTSEL0 = 0x00;	 //f�r  TX Out
  	PORT_PAGE = 0;
  	P1_DIR = P1_DIR | 0x02;		// Direction TXD   P1.1
  	SCON = 0x50;	// Mode 1 , Receive enable
	// Baudrate :
  	BCON =  0;		// damit BG Write-enable
  	BG	 =	156;	// Baud Rate Reload Register f�r 9600 Baud
...
// Duty Cycle �ber AN5 einstellbar
//******************************************************************
...
void t13_init(void)
{
	...
	//CCU6_PSLR = 0x00;		// Ausg�nge im passiven Zustand = 0
